...
-- Step 1: Add new columns to existing bills table
...
-- Step 2: Create Clients table first
...
-- Step 3: Now add the client_id column to bills (after clients table exists)
ALTER TABLE bills ADD COLUMN IF NOT EXISTS client_id UUID REFERENCES clients(id) ON DELETE SET NULL;

-- Step 4: Create Bill Notes table
...
-- Step 6: Create User Actions table for tracking
...
-- Step 12: Insert sample contacts
...
-- Step 13: Update some bills with client relationships
...
-- Step 14: Insert sample bill notes
